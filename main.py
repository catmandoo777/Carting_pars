...
def get_html(URL, HEADER, time_sleep):
    print(URL)
...
def for_json_first(url, name, data):
    data_row = {url: data}
    if path.exists(name):
...
def main():
    ...
    for i,j in url_map.items():
        ...
        temp_row[i] = pages
        print(temp_row)
        with open('data_first.json', 'w') as file:
            json.dump(temp_row, file, indent=4, ensure_ascii=False)
